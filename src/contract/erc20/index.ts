...
export default class ERC20 extends VerifiedContract {
    ...
    public async balanceOf(_account: string, options?:{ gasPrice: number, gasLimit: number}): any {
        await this.validateInput(DATATYPES.ADDRESS, _account)
        return this.callContract(FUNCTIONS.BALANCE, _account, options)
    }

    public async transfer(_recipient: string, _amount: string, options?: { gasPrice: number, gasLimit: number }): any {
