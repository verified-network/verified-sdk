...
import { SecuritiesFactory, Security, contractAddress } from "../index";
...
describe("Polygon Mumbai Testnet Tests", () => {
  ...
  describe("SecurityFactories Test", () => {
    ...
    it("it should Issue Security, add balance(mint 100) security token to user's account and transfer 1 security token from user account to another account", async () => {
        ...
        .then(async (res: any) => {
          ...
          if(hash.length > 0) {
            securityFactoryContract.notifySecuritiesAdded(async (evnt: any) => {
              ...
              assert.notEqual(security.length, 0);
              if (security !== "") {
                it(`should test add Balance(mint 100) of ${security} security token `, async() => {
                  await securityFactoryContract
                  .addBalance(
                    security,
                    zeroAddress,
                  ...
                  .then(async (res: any) => {
                    ...
                    if(addBalanceHash.length > 0) {
                      it(`should test sending 1 ${security} token`, async () => {
                        const securityContract = new Security(signer, security);
